...
create_package_from_source() {
    ...
    local package_dir="${PACKAGES_DIR}/hardware/${package_name}"
    mkdir -p "$package_dir"
    
    # Create standard directories
    for dir in "assets" "examples" "src" "tests"; do
        mkdir -p "$package_dir/$dir"
        echo "# hardware/$package_name - $dir" > "$package_dir/$dir/README.md"
    done
...
# Process hardware package
echo -e "${BLUE}Starting hardware package refactoring...${NC}"
echo ""

# Find all .smash files in the hardware/src directory
for source_file in "${HARDWARE_DIR}/src"/*.smash; do
    if [ -f "$source_file" ] && [ "$(basename "$source_file")" != "index.smash" ]; then
        create_package_from_source "$source_file"
    fi
done
